    ...
    public class CADVehiculo
    {
        ...
        public bool readVehiculo(ENVehiculo en)
        {
            ...
            try
            {
                ...
                en.precioVehiculo = int.Parse(dr["precio"].ToString());
                en.vendedorVehiculo = dr["cif"].ToString();
                en.compradorVehiculo = dr["nif"].ToString();
                en.imagenVehiculo = dr["imagen"].ToString();
                transaction = true;
            }
            catch (Exception ex)
            {
        ...
        public ArrayList ListarVehiculosPorEmpresa(ENVehiculo en)
        {
            SqlConnection c = new SqlConnection(constring);
            c.Open();
            SqlCommand com2 = new SqlCommand("Select matricula from Vehiculo where cif='" + en.vendedorVehiculo + "'", c);
            SqlDataReader dr2 = com2.ExecuteReader();
            while (dr2.Read())
            {
                string matricula = dr2["matricula"].ToString();
                lista2.Add(matricula);
            }
            dr2.Close();
            c.Close();
            return lista2;
